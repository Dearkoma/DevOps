...
public class BuildService {
    ...
    @Value("${devops.pipeline.k8s.host-ip:auto}")
    private String k8sHostIp;
    ...
    private String generateDefaultK8sDeployment(Project project) {
        ...
        return "apiVersion: apps/v1\n" +
                ...
                "          env:\n" +
                "            # 覆盖数据库连接地址（Pod 内 localhost ≠ 宿主机）\n" +
                "            - name: SPRING_DATASOURCE_URL\n" +
                "              value: jdbc:mysql://" + hostIp + ":3306/devops_platform?useUnicode=true&characterEncoding=utf-8&useSSL=false&serverTimezone=Asia/Shanghai&allowPublicKeyRetrieval=true&createDatabaseIfNotExist=true\n" +
    ...
    /**
     * 解析宿主机 IP，供 K8s Pod 访问宿主机上的 MySQL 等服务
     * 优先级：配置值 > 自动检测（WSL 网关 / Docker 内部地址）
     */
    private String resolveHostIp() {
        // 1. 手动指定
        if (k8sHostIp != null && !k8sHostIp.isBlank() && !"auto".equalsIgnoreCase(k8sHostIp)) {
            return k8sHostIp;
        }
        // 2. 自动检测 WSL 网关（Rancher Desktop 场景）
        try {
            ...
            if (p.waitFor(3, java.util.concurrent.TimeUnit.SECONDS) && p.exitValue() == 0) {
                String gw = new String(p.getInputStream().readAllBytes()).trim();
                if (!gw.isEmpty() && !"0.0.0.0".equals(gw)) {
                    log.info("检测到 WSL 网关 IP: {}", gw);
                    return gw;
                }
            }
        } catch (Exception ignored) {}
        ...
        try {
            ...
            ProcessBuilder pb = new ProcessBuilder("bash", "-c",
                    "ping -c 1 -W 1 host.docker.internal 2>/dev/null >/dev/null && echo 'ok'");
            pb.redirectErrorStream(true);
            Process p = pb.start();
            if (p.waitFor(3, java.util.concurrent.TimeUnit.SECONDS) && p.exitValue() == 0) {
                String out = new String(p.getInputStream().readAllBytes()).trim();
                if ("ok".equals(out)) {
                    log.info("检测到 Docker Desktop，使用 host.docker.internal");
                    return "host.docker.internal";
                }
            }
        } catch (Exception ignored) {}
        // 4. 回退：尝试常见 WSL 网关段
        log.warn("无法自动检测宿主机 IP，使用默认值 172.24.176.1");
        return "172.24.176.1";
    }
    ...
    /**
     * 列出项目 workspace 下的文件和目录
     * @param projectId 项目 ID
     * @param relativePath 相对于项目 workspace 的路径，空字符串表示根目录
     * @return 文件/目录信息列表
     */
    public List<Map<String, Object>> listProjectFiles(Long projectId, String relativePath) {
        Project project = projectRepository.findById(projectId)
                .orElseThrow(() -> new RuntimeException("项目不存在: " + projectId));
        String workspace = getProjectWorkspace(project);
        Path baseDir = Paths.get(workspace);
        ...
        if (relativePath != null && !relativePath.isBlank()) {
            targetDir = baseDir.resolve(relativePath).normalize();
        ...
        try (var stream = Files.list(targetDir)) {
            ...
            for (Path entry : entries) {
                ...
                info.put("name", name);
                info.put("isDirectory", isDir);
                try {
                    info.put("size", Files.size(entry));
                    info.put("lastModified", Files.getLastModifiedTime(entry).toInstant().toString());
                } catch (IOException e) {
                    info.put("size", 0);
                ...
                if (!isDir) {
                    ...
                    if (lowerName.endsWith(".java") || lowerName.endsWith(".js") || lowerName.endsWith(".jsx")
                            || lowerName.endsWith(".ts") || lowerName.endsWith(".tsx") || lowerName.endsWith(".css")
                            || lowerName.endsWith(".html") || lowerName.endsWith(".py") || lowerName.endsWith(".go")
                            || lowerName.endsWith(".xml") || lowerName.endsWith(".yml") || lowerName.endsWith(".yaml")
                            || lowerName.endsWith(".json") || lowerName.endsWith(".properties")
                            || lowerName.endsWith(".sql") || lowerName.endsWith(".md") || lowerName.endsWith(".txt")) {
                        info.put("type", "text");
                    } else if (lowerName.endsWith(".jar") || lowerName.endsWith(".war")) {
                        info.put("type", "archive");
                    } else if (lowerName.endsWith(".png") || lowerName.endsWith(".jpg") || lowerName.endsWith(".jpeg")
                            || lowerName.endsWith(".gif") || lowerName.endsWith(".svg") || lowerName.endsWith(".ico")) {
                        info.put("type", "image");
            ...
            log.warn("列出项目文件失败: projectId={}, path={}", projectId, relativePath, e);
        }
        return files;
    }

    /**
     * 读取项目 workspace 下的文本文件内容
     * @param projectId 项目 ID
     * @param relativePath 文件相对路径
     * @return 文件内容字符串，如果文件过大则返回截断版本
     */
    public Map<String, Object> readProjectFile(Long projectId, String relativePath) {
        Project project = projectRepository.findById(projectId)
                .orElseThrow(() -> new RuntimeException("项目不存在: " + projectId));
        String workspace = getProjectWorkspace(project);
        Path baseDir = Paths.get(workspace);
        ...
        if (!Files.exists(baseDir)) {
            throw new RuntimeException("项目工作目录尚未创建，请先触发一次构建");
        }
        ...
        if (Files.isDirectory(filePath)) {
            throw new RuntimeException("路径指向一个目录，而非文件");
        }

        // 检测文件大小：超过 1MB 的二进制文件拒绝读取
        long fileSize;
        try {
            fileSize = Files.size(filePath);
        } catch (IOException e) {
            throw new RuntimeException("无法读取文件大小");
        }

        // 判断是否为文本文件
        String fileName = filePath.getFileName().toString().toLowerCase();
        boolean isText = fileName.endsWith(".java") || fileName.endsWith(".js") || fileName.endsWith(".jsx")
                || fileName.endsWith(".ts") || fileName.endsWith(".tsx") || fileName.endsWith(".css")
                || fileName.endsWith(".html") || fileName.endsWith(".py") || fileName.endsWith(".go")
                || fileName.endsWith(".xml") || fileName.endsWith(".yml") || fileName.endsWith(".yaml")
                || fileName.endsWith(".json") || fileName.endsWith(".properties") || fileName.endsWith(".sql")
                || fileName.endsWith(".md") || fileName.endsWith(".txt") || fileName.endsWith(".log")
                || fileName.endsWith(".env") || fileName.endsWith(".gradle") || fileName.endsWith(".gitignore")
                || fileName.endsWith(".dockerfile") || fileName.endsWith(".sh") || fileName.endsWith(".bat")
                || fileName.equals("dockerfile") || fileName.equals("makefile") || fileName.equals(".editorconfig");

        if (!isText && fileSize > 100 * 1024) {  // 100KB 以上的非文本文件
            throw new RuntimeException("无法预览此类型文件 (" + fileName + ")，请下载后查看");
        }
        ...
        try {
            long maxReadSize = 500 * 1024;
            if (fileSize > maxReadSize) {
                // 大文件截断读取
                byte[] bytes = Files.readAllBytes(filePath);
                String content = new String(bytes, 0, (int) maxReadSize);
                result.put("content", content);
                result.put("truncated", true);
                result.put("totalSize", fileSize);
            } else {
                result.put("content", Files.readString(filePath));
    ...
    // ===================== 多环境文件操作（workspace / Docker / K8s） =====================

    /**
     * 列出项目文件 —— 根据 context 切换不同环境
     * @param context  "workspace" (默认/构建前只读), "docker", "k8s"
     */
    public List<Map<String, Object>> listProjectFiles(Long projectId, String relativePath, String context) {
        if (context == null || context.isBlank() || "workspace".equalsIgnoreCase(context)) {
            return listProjectFiles(projectId, relativePath);
        }
        Project project = projectRepository.findById(projectId)
                .orElseThrow(() -> new RuntimeException("项目不存在: " + projectId));

        if ("docker".equalsIgnoreCase(context)) {
            return listDockerFiles(project, relativePath);
        } else if ("k8s".equalsIgnoreCase(context)) {
            return listK8sFiles(project, relativePath);
        }
        throw new RuntimeException("不支持的环境: " + context);
    }

    /**
     * 读取文件内容 —— 根据 context 切换不同环境
     */
    public Map<String, Object> readProjectFile(Long projectId, String relativePath, String context) {
        if (context == null || context.isBlank() || "workspace".equalsIgnoreCase(context)) {
        ...
        if ("docker".equalsIgnoreCase(context)) {
            return readFileFromExec(project, relativePath, "docker");
        } else if ("k8s".equalsIgnoreCase(context)) {
            return readFileFromExec(project, relativePath, "k8s");
        }
        throw new RuntimeException("不支持的环境: " + context);
    }

    /**
     * 写入文件内容 —— 仅 Docker/K8s 支持编辑
     * workspace 环境下禁止写入
     */
    public Map<String, Object> writeProjectFile(Long projectId, String relativePath, String content, String context) {
        if (context == null || context.isBlank() || "workspace".equalsIgnoreCase(context)) {
            throw new RuntimeException("工作空间文件为只读模式，请切换到 Docker 或 K8s 环境进行编辑");
        }
        Project project = projectRepository.findById(projectId)
                .orElseThrow(() -> new RuntimeException("项目不存在: " + projectId));

        if ("docker".equalsIgnoreCase(context)) {
            return writeFileViaExec(project, relativePath, content, "docker");
        } else if ("k8s".equalsIgnoreCase(context)) {
            return writeFileViaExec(project, relativePath, content, "k8s");
        }
        throw new RuntimeException("不支持的环境: " + context);
    }

    /** Docker: 列出 workspace 源码文件（可编辑模式） */
    private List<Map<String, Object>> listDockerFiles(Project project, String relativePath) {
        // Docker 模式下展示工作目录源码，标记为可编辑
        List<Map<String, Object>> files = listProjectFiles(project.getId(), relativePath);
    ...
    /** K8s: 列出 workspace 源码文件（可编辑模式） */
    private List<Map<String, Object>> listK8sFiles(Project project, String relativePath) {
        // K8s 模式下展示工作目录源码，标记为可编辑
        List<Map<String, Object>> files = listProjectFiles(project.getId(), relativePath);
        for (Map<String, Object> f : files) {
            f.put("writable", true);
        }
    ...
    /** 通用：通过 exec 命令列目录 */
    private List<Map<String, Object>> listFilesViaExec(String id, String namespace, String relativePath, String type) {
        String execPath = "/app"; // 默认查看 /app 目录
        if (relativePath != null && !relativePath.isBlank()) {
            execPath = "/app/" + relativePath;
        }
        // 安全：防止路径穿越
        execPath = Paths.get(execPath).normalize().toString().replace("\\", "/");
        if (!execPath.startsWith("/app")) execPath = "/app";

        List<Map<String, Object>> files = new ArrayList<>();
        try {
            ProcessBuilder pb;
            if ("docker".equals(type)) {
                pb = new ProcessBuilder(dockerCommand, "exec", id,
                        "sh", "-c", "ls -la " + shellQuote(execPath) + " 2>/dev/null || echo ''");
            } else {
                pb = new ProcessBuilder("kubectl", "exec", "-n", namespace, id, "--",
                        "sh", "-c", "ls -la " + shellQuote(execPath) + " 2>/dev/null || echo ''");
            }
            pb.redirectErrorStream(true);
            Process p = pb.start();
            String output = new String(p.getInputStream().readAllBytes());
            if (!p.waitFor(10, java.util.concurrent.TimeUnit.SECONDS)) { p.destroyForcibly(); return files; }

            if (output.isBlank() || output.trim().isEmpty()) return files;

            // 解析 ls -la 输出
            for (String line : output.split("\\R")) {
                line = line.trim();
                if (line.isEmpty() || line.startsWith("total ")) continue;
                // 格式: drwxr-xr-x 2 root root 4096 Jan 1 2025 dirname
                //       -rw-r--r-- 1 root root 1234 Jan 1 2025 filename
                ...
                info.put("size", size);
                info.put("lastModified", null);
                if (!isDir) info.put("type", detectFileType(name));
                info.put("writable", true);  // 容器内文件可编辑
                files.add(info);
            }
            // 排序：目录在前，按名称排序
            files.sort((a, b) -> {
            ...
            log.warn("exec ls 失败 ({}): {}", type, e.getMessage());
        }
        return files;
    }

    /** 通用：通过 exec 读取文件内容 */
    private Map<String, Object> readFileFromExec(Project project, String relativePath, String type) {
        // Docker/K8s 模式下读取 workspace 源码文件
        return readProjectFile(project.getId(), relativePath);
    }

    /** 通用：写入文件到 workspace（Docker/K8s 编辑模式） */
    private Map<String, Object> writeFileViaExec(Project project, String relativePath, String content, String type) {
        String workspace = getProjectWorkspace(project);
        Path baseDir = Paths.get(workspace);
        ...
        result.put("path", relativePath);
        result.put("message", "文件已保存到工作目录: " + relativePath);
        return result;
    }

    /** 查找项目对应的 Docker 容器 ID */
    private String findDockerContainer(Project project) {
        try {
            // 查找标签 project=<code> 的容器
            ProcessBuilder pb = new ProcessBuilder(dockerCommand, "ps",
                    "--filter", "label=project=" + project.getCode(),
                    "--format", "{{.ID}}");
            pb.redirectErrorStream(true);
            Process p = pb.start();
            ...
            if (!output.isEmpty()) {
                return output.split("\\R")[0].trim(); // 返回第一个
            }

            // 备选：通过镜像名查找
            String imagePrefix = project.getCode().toLowerCase();
            pb = new ProcessBuilder(dockerCommand, "ps",
                    "--filter", "ancestor=" + imagePrefix + ":latest",
                    "--format", "{{.ID}}");
            p = pb.start();
            output = new String(p.getInputStream().readAllBytes()).trim();
            p.waitFor(8, java.util.concurrent.TimeUnit.SECONDS);
            if (!output.isEmpty()) return output.split("\\R")[0].trim();

            // 再试：通过名称模糊匹配
            pb = new ProcessBuilder(dockerCommand, "ps",
                    "--filter", "name=" + project.getCode(),
                    "--format", "{{.ID}}");
            p = pb.start();
            output = new String(p.getInputStream().readAllBytes()).trim();
            p.waitFor(8, java.util.concurrent.TimeUnit.SECONDS);
            return output.isEmpty() ? null : output.split("\\R")[0].trim();
        } catch (Exception e) {
            log.warn("查找 Docker 容器失败: project={}, error={}", project.getCode(), e.getMessage());
            return null;
        }
    }

    /** 查找项目对应的 K8s Pod 名称和 namespace */
    private String[] findK8sPod(Project project) {
        try {
            // 查找标签 app=<project-code> 的 Pod
            ProcessBuilder pb = new ProcessBuilder("kubectl", "get", "pods",
                    "-n", k8sNamespace,
                    "-l", "app=" + project.getCode(),
                    "-o", "jsonpath={.items[0].metadata.name}");
            pb.redirectErrorStream(true);
            Process p = pb.start();
            String name = new String(p.getInputStream().readAllBytes()).trim();
            p.waitFor(8, java.util.concurrent.TimeUnit.SECONDS);
            if (!name.isEmpty()) return new String[]{name, k8sNamespace};

            // 备选：通过 deployment 名查找
            String deployName = project.getCode() + "-" + project.getId();
            pb = new ProcessBuilder("kubectl", "get", "pods",
                    "-n", k8sNamespace,
                    "-l", "app=" + deployName,
                    "-o", "jsonpath={.items[0].metadata.name}");
            p = pb.start();
            name = new String(p.getInputStream().readAllBytes()).trim();
            p.waitFor(8, java.util.concurrent.TimeUnit.SECONDS);
            return name.isEmpty() ? null : new String[]{name, k8sNamespace};
        } catch (Exception e) {
            log.warn("查找 K8s Pod 失败: project={}, error={}", project.getCode(), e.getMessage());
            return null;
        }
    }

    /** shell 参数安全引用（仅用于单段路径，不用于拼接的命令） */
    ...
    private String detectFileType(String name) {
        ...
        if (lower.endsWith(".java") || lower.endsWith(".js") || lower.endsWith(".jsx")
                || lower.endsWith(".ts") || lower.endsWith(".tsx") || lower.endsWith(".css")
                || lower.endsWith(".html") || lower.endsWith(".py") || lower.endsWith(".go")
                || lower.endsWith(".xml") || lower.endsWith(".yml") || lower.endsWith(".yaml")
                || lower.endsWith(".json") || lower.endsWith(".properties")
                || lower.endsWith(".sql") || lower.endsWith(".md") || lower.endsWith(".txt")
                || lower.endsWith(".env") || lower.endsWith(".gradle") || lower.endsWith(".gitignore")
                || lower.endsWith(".sh") || lower.endsWith(".bat") || lower.endsWith(".conf")
                || lower.endsWith(".ini") || lower.endsWith(".cfg") || lower.endsWith(".toml")
                || lower.equals("dockerfile") || lower.equals("makefile") || lower.equals(".editorconfig")) {
            return "text";
        } else if (lower.endsWith(".jar") || lower.endsWith(".war") || lower.endsWith(".tar.gz") || lower.endsWith(".zip")) {
            return "archive";
