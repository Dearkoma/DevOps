...
1. **"DevOps" / "D项目" = 本项目本身**。用户提到"DevOps"或"D项目"时即指这个毕业设计项目，不是泛指 DevOps 概念。
   **"A项目"** = D 项目（DevOps 平台）内部创建/管理/部署的下游项目（如 `proj-1782825767454`）。
2. **代码操作必须可恢复**。通过 git 保障：修改前确保工作区干净或先提交存档；提供修改方案时附带回退方式。
3. **修改可由 AI 直接执行**（2026-06-30 更新）。AI 可以直接改代码/文件并 git 提交。用户通过 IDEA 运行后端。
4. **AI 可自行启动/停止后端验证**（2026-07-01）。改完代码后 AI 可以通过 CLI（mvn package + java -jar）自行启动后端验证功能是否正常。验证完成后必须**立即停止后端进程**，然后用户再去 IDEA 中观察。不要在验证成功后让后端一直跑着。
...
## 可恢复机制
- 当前最新提交 `3541fc7`（2026-07-01 03:15 侧边栏数字气泡）。
